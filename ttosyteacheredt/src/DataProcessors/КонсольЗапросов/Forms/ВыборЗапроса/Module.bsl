...
Процедура СравнитьРезультатыЗапросов(Команда)
	...
	Если ВыделенныеЗапросы.Количество() <> 2 Тогда
		Предупреждение(НСтр("ru = 'Для сравнения необходимо выбрать только 2 запроса", "Объект'"));
		Возврат;
	Иначе
	...
	КонецЕсли;	
	#Иначе
	 	Предупреждение(НСтр("ru = 'Сравнивать результаты можно только в режиме толстого клиента.", "Объект'"));
	#КонецЕсли
КонецПроцедуры
...
Процедура ЗагрузитьЗапросыИзФайла(ДвоичныеДанные)
	...
	ЗаполнитьЗапросыИПармаетрыИзОбъектаВнешнейОбработки(ОбъектВнешнейОбработки);
